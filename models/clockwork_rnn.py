...
class ClockworkRNN(object):
    ...
    def _build_model(self):
        ...
        x_list = [tf.squeeze(x, axis=[1]) for x in tf.split(axis=1, num_or_size_splits=self.config.num_steps, value=self.inputs, name="inputs_list")]
        ...
        with tf.variable_scope("clockwork_cell") as scope:
            ...
            # self.state = tf.get_variable(name="state", shape=[self.config.batch_size, self.config.num_hidden],
            #                              initializer=tf.zeros_initializer, trainable=False)
            zeros = tf.zeros([self.config.batch_size, self.config.num_hidden])
            self.state = tf.Variable(zeros, trainable=False, name="state")
            ...
            for time_step in range(self.config.num_steps):
                ...
                y_update = tf.add(WH_y, WI_x, name="update_state")
                y_update = activation_hidden(y_update)

                # Copy the updates to the cell state
                self.state = tf.concat(axis=1, values=[y_update, tf.slice(self.state, [0, group_index], [-1,-1])])
    ...
    def _build_summary_ops(self):
        ...
        self.train_summary_op = tf.summary.merge([training_summaries, self.gradient_summaries_merged])
